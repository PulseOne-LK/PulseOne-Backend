...
public interface WaitingRoomRepository extends JpaRepository<WaitingRoom, UUID> {
    ...
    /**
 * Get average wait time in minutes for a doctor on a specific date
 * Only includes patients who have been called and appointments that are completed or in progress
 */
@Query(value = "SELECT AVG(EXTRACT(EPOCH FROM (wr.called_at - wr.checked_in_at))/60) " +
       "FROM waiting_room wr " +
       "JOIN appointments a ON wr.appointment_id = a.appointment_id " +
       "WHERE a.doctor_id = :doctorId " +
       "AND a.appointment_date = :date " +
       "AND wr.called_at IS NOT NULL " +
 ...
 * Returns: [totalCheckedIn, stillWaiting, inConsultation, completed, avgWaitMinutes]
 */
@Query(value = "SELECT " +
       "COUNT(wr.waiting_room_id) as total_checked_in, " +
       "COUNT(CASE WHEN a.status = 'CHECKED_IN' THEN 1 END) as still_waiting, " +
       "COUNT(CASE WHEN a.status = 'IN_PROGRESS' THEN 1 END) as in_consultation, " +
       "COUNT(CASE WHEN a.status = 'COMPLETED' THEN 1 END) as completed, " +
       "AVG(CASE WHEN wr.called_at IS NOT NULL " +
