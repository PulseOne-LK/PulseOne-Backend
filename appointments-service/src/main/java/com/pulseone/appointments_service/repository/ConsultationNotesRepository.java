...
public interface ConsultationNotesRepository extends JpaRepository<ConsultationNotes, UUID> {
    ...
    @Query("SELECT cn FROM ConsultationNotes cn " +
           ...
           "AND (a.appointmentDate + INTERVAL cn.followUpInDays DAY) <= :currentDate " +
    ...
    @Query("SELECT cn FROM ConsultationNotes cn " +
           ...
           "WHERE cn.followUpRequired = true " +
           "AND cn.followUpInDays IS NOT NULL " +
           "AND cn.followUpInDays <= 7 " +
           "AND (a.appointmentDate + INTERVAL cn.followUpInDays DAY) BETWEEN :currentDate AND :nextWeek " +
           "ORDER BY (a.appointmentDate + INTERVAL cn.followUpInDays DAY)")
